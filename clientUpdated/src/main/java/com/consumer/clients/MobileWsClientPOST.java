...
public class MobileWsClientPOST {
	int port = 8081;
	...
	public MobileWsClientPOST(int port) {
		this.port = port;
	...
	public static void main(String[] args) {
		String response = new MobileWsClientPOST().postUserDetails();
	}

	public String postUserDetails() {
		String url = String.format("Http://localhost:%d/users", port);
		String payload = "{" + "\"firstName\":\"Salman\"," + "\"lastName\":\"S\"," + "\"email\":\"sms9211@gmail.com\""
				+ "}";
		...
		// try {
		// //HttpResponse
		// r=Request.Get(url).addHeader("Accept","application/json").execute().returnResponse();
